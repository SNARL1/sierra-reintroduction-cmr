...
The full factorization of the joint distribution of data and parameters is provided in \hyperref[joint-distribution]{Appendix S1: Joint distribution factorization}.
...
We used the forward algorithm [@zucchini2016hidden] to circumvent the need to sample from the discrete space of true individual states (see \hyperref[forward-algorithm]{Appendix S2: Forward algorithm description} for details).
...
The estimated correlation over time between mean log Bd loads at the two study sites was `r median(unlist(bd_load_correlations)) %>% round(n_digit)` (`r quantile(unlist(bd_load_correlations), .05) %>% round(n_digit)`, `r quantile(unlist(bd_load_correlations), .95) %>% round(n_digit)`)  (Figure \ref{mean-bd-loads}A). 
...
As a consequence, when mean Bd loads were high, prevalence also tended to be high (Figure \ref{mean-bd-loads}). 
...
\subsection*{Appendix S1: Joint distribution factorization}
...
\subsection*{Appendix S2: Forward algorithm description}
