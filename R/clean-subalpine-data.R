...
d <- read_csv('data/subalpine_captures_2008-2017.csv') %>%
...
translocations <- read_csv('data/subalpine_introductions_2008-2017.csv') %>%
  mutate(pit_tag_id = parse_character(pit_tag_id))
...
# while we don't have survey_target or more surveys/year, draw from model to
...
d <- d %>%
  select(capture_date, prim_period, pit_tag_id, swab_id, bd_load) %>%
...
detection_covs <- sec_date_df %>%
  ...
  mutate(j = 1:n(),
         ...
         air_temp_c = ifelse(is.na(air_temp_c),
                             ...
                             air_temp_c))
# above mutation replaces NA values with the mean
